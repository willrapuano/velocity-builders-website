...
import {
  formatCaseStudyMetric,
  type CaseStudyMetricUnit,
...
type CaseStudyCard = { _id: string; title: string; slug: { current: string }; summary: string; publicClientLabel?: string | null; verifiedMetrics?: Array<{ key: string; label: string; value: number; unit: CaseStudyMetricUnit }> };

export default async function CaseStudiesPage() {
  const studies = await client.fetch<CaseStudyCard[]>(caseStudiesQuery, {}, { next: { revalidate: 300 } });
  const allowedStudies = studies.filter(
    (study) => !containsBlockedMlsDisplayLanguage(study)
  );
  return <div className="bg-slate-950 text-white"><section className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
    ...
    {allowedStudies.length ? allowedStudies.map((study) => <article key={study._id} className="rounded-2xl border border-slate-200 p-7">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3">{study.verifiedMetrics?.slice(0, 4).map((metric) => <div key={metric.key} className="rounded-xl bg-slate-50 p-4"><p className="font-mono text-xl font-semibold">{formatCaseStudyMetric(metric.value, metric.unit)}</p><p className="mt-1 text-xs text-slate-500">{metric.label}</p></div>)}</div>
